...
const AdminAffiliateNetwork = () => {
  ...
  const updateRate = async () => {
    ...
    try { setActionLoading("rate"); const response = await request("/admin/affiliate-settings", { method: "PUT", body: JSON.stringify({ globalAffiliateRate: Number(rate) }) }); setData((current) => ({ ...current, globalRate: response?.settings?.globalAffiliateRate ?? Number(rate) })); alert("Commission rate updated."); } catch (err) { alert(err.message || "Failed to update rate"); } finally { setActionLoading(""); }
  };
